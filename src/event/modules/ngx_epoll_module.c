...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
...
#if (NGX_HAVE_EPOLLRDHUP)
        ngx_epoll_test_rdhup(cycle); //这里面为ngx_use_epoll_rdhup赋值
#endif
    }
    ...
    ngx_event_flags = NGX_USE_CLEAR_EVENT    //边缘触发模式
#else
...
#if (NGX_HAVE_SYS_EVENTFD_H)
    notify_fd = eventfd(0, 0);   //eventfd 这里没有设置成非阻塞
#else
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    //data.ptr指向ngx_connection_t,ngx_connection_t的最低位表示ev->instance,用于stale事件
