...
ngx_event_accept(ngx_event_t *ev)
{
...
#endif
    //什么情况下会设置accept事件的timedout(当accept因为打开文件过多而失败时)
    if (ev->timedout) {
    ...
    do {
        ...
        if (s == (ngx_socket_t) -1) {
            ...
            if (err == NGX_ECONNABORTED) {
                if (ngx_event_flags & NGX_USE_KQUEUE_EVENT) {
                    ev->available--;  //bsd类系统上面知道有多少未accept的连接吗?
                }
...
#if (NGX_STAT_STUB)
        (void) ngx_atomic_fetch_add(ngx_stat_accepted, 1); //ngx_stat_accepted位于共享内存中
#endif
        ...
        c->pool = ngx_create_pool(ls->pool_size, ev->log);   //每个连接有单独的pool
        if (c->pool == NULL) {
        ...
        //ngx_inherited_nonblocking表示是否有使用accept4,使用accept4就可以不调用ngx_nonblocking
        ...
        *log = ls->log;
        ...
        if (ev->deferred_accept) {
            rev->ready = 1;  //deferred_accept的情况下,到这里就肯定有数据可读了
#if (NGX_HAVE_KQUEUE)
            rev->available = 1;
#endif
        }
...
ngx_close_accepted_connection(ngx_connection_t *c)
{
    ...
    fd = c->fd;
    c->fd = (ngx_socket_t) -1;
    //c->shared是啥意思?(ngx_event_recvmsg函数里面会设置这个字段为1,即udp的情况)
