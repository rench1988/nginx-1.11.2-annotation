...
#endif
{
    ...
    if (level != NGX_LOG_DEBUG && log->handler) {
        p = log->handler(log, p, last - p); //log->handler为日志输出增加指定内容
    }
...
ngx_log_open_default(ngx_cycle_t *cycle)
{
    ...
    log->file = ngx_conf_open_file(cycle, &error_log);  //并没有真正打开文件,只是为文件名字段赋值
    if (log->file == NULL) {
        return NGX_ERROR;
    }
