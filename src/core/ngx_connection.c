...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    //files代表什么? cycle->files只有在ngx_event_flags包含NGX_USE_FD_EVENT时才会使用
    ...
    rev->index = NGX_INVALID_INDEX;   //index代表啥?
    wev->index = NGX_INVALID_INDEX;

    rev->data = c;
    wev->data = c;

    wev->write = 1;    //wev->write置为1代表啥?
    ...
    ngx_uint_t port)
{
    ...
    //addr == 0表示listen的地址是*:port,因此需要getsockname获取本地地址吗?
    if (addr == 0) {
